        ...
        switch(msg.type) {
            ...
            case "tokenfaild":
            {
                ...
                case "addanewuser"://这个是完成http用户添加后就在world中建立一个BallMan，不论新用户是否连接这个用户都会显示在场景中
                {
                    if(userid=="world")//world用户得知有新用户建立,认为新建的用户肯定没有下属单位？《-新建的用户之前由java生成还未接触过js代码
                    {
                        ...
                        arr_webusers.push({units:[],id:msg.id,uuid:msg.uuid,jsonstate:msg.jsonstate,mesh:player});
            ...
            case "getuserstate_init"://非world用户建立WS连接时，world用户为他们提供世界状态
            {
                if(userid=="world")
                {
                    ...
                    for(key in MyGame.arr_webplayers)
                    //for(var i=0;i<len;i++)
                    {
            ...
            case "quituser":
            {
                if(userid=="world")
                {
                    ...
                    {
                        //首先还要把user的所有card清理掉
                        if(user.arr_unitss)//规定这个消息删除的必定是webuser的单位，自己的单位列表存在MyGame里
                        {
                            var len=arr_unitss.length;
                            for(var i=0;i<len;i++)
                            {
                                arr_unitss[i].dispose();
                            }
                        }
                        user.dispose();
                        user=null;
            ...
            case "bc_quituser":
            {
                if(userid!="world") {
                    ...
                    if (id == MyGame.userid)//自己就是被登出的用户
                    {
                        ...
                        var user = MyGame.arr_webplayers[id];
                        if (user && user.mesh) {
                            if(user.arr_unitss)//规定这个消息删除的必定是webuser的单位
                            {
                                var len=arr_unitss.length;
                                for(var i=0;i<len;i++)
                                {
            ...
            case "finish_catchcard":
            {
                if(userid!="world")
                {
                    ...
                    {
                        var user=MyGame.arr_webplayers[msg.userid];
                        var arr_units=MyGame.arr_worldunits;
                        var len=arr_units.length;
                        for(var i=0;i<len;i++)
                        {
                            var unit=arr_units[i];
                            if(unit.uuid==msg.uuid)
                            {
                                var card=unit;
                                ChangeCardsBelone("world", msg.userid, card);
                                card.state="inhand";
                                user.cardinhand.push(card);
                                card.mesh.parent =user.handpoint;
                                ArrangeHandCard(user.cardinhand);
                                break;
                            }
                        }
                    }
                    ...
                    for(var i=0;i<len;i++)
                    {
                        ...
                        if(unit.uuid==card_uuid)
                        {
                            ...
                            card.mesh.physicsImpostor = new BABYLON.PhysicsImpostor(card.mesh, BABYLON.PhysicsImpostor.BoxImpostor
                               , { mass: 1, restitution: 0.5 ,friction:0.5,move:true}, scene);
                            ...
                            for(var j=0;j<len2;j++)
                            {
                                if(card_uuid==ballman.cardinhand[i].uuid)
                                {
            ...
            case "bc_playoutcard":
            {
                if(userid!="world")
                {
                    ...
                    //if(userid==msg.userid)//是自己的卡牌
                    {
                        ...
                        {
                            arr_units = MyGame.arr_units;
                            ballman = MyGame.player;//.mesh.ballman;//真的取到ballman这一层就没有各种arr了！！
                        } else {
                            ballman = MyGame.arr_webplayers[webuser_id];
                            arr_units = ballman.arr_units;
                        }
                        var len = arr_units.length;
                        for (var i = 0; i < len; i++) {
                            var unit = arr_units[i];
...
function sendMessage(str_json) {
    if(MyGame.wsConnected) {
        ...
        //logWs("与服务器断开连接了，刷新重新连接~");//这样的设定会一直不断的log！
    }
