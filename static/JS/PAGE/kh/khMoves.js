...
function CamerasFollowActor(object)
{
    if(object.prototype=CameraMesh)
    {
        ...
        if(MyGame.flag_view=="first_lock"||MyGame.flag_view=="first_ani")//动画时相机也要跟随
        {
            ...
            //除了自己以外，webuser中的每个对象也要跟随！！！！
            var arr=MyGame.arr_webplayers;
            for(var key in arr)
            {
                if(key!="world")
                {
                    var obj=arr[key];
                    obj.head.position=obj.mesh.position.clone();//我没有设置head是ballman的子元素，所以位置和姿态要手动修改
                    obj.head.rotation=obj.mesh.rotation.clone();//因为要保留添加物理外壳的可能性
                }
            }
