...
import com.websocket.PlaygroundApplication;
import com.websocket.pojo.WsMessage;
import com.websocket.pojo.WsSession;
...
public class WebSocketHandler {
    ...
    {
        try {
            jdbcTemplate = new JdbcTemplate(PlaygroundApplication.ds);
            ...
            switch (type) {
                case "setuserid": // 在连接的身份对象中记录这个连接所属的用户的身份信息-》这个数据库查询改为world用户专用
                {
                    ...
                    if (list.size() == 1) {
                        ...
                        //查出用户的所有单位，查出所有其他用户，查出所有其他用户暴露给本用户的单位
                        str_sql = "select a.uuid,a.jsonstate,a.ispublic,b.name,b.jsonp,b.mainpic,b.mainback,b.comment  from  tb_userunit a,tb_unittype b \n" +
                                "where a.owneruuid=? and a.sfky=1 and pgid=?  and  a.unittype=b.uuid";
                        List<Map<String, Object>> list2 = jdbcTemplate.queryForList(str_sql, token, str_pgid);
                        List<JSONObject> arr_data = new ArrayList<JSONObject>();
                        int len = list2.size();//对于这个用户的每一个单位
                        int i = 0;
                        for (i = 0; i < len; i++) {
                        ...
                        str_sql = "SELECT uuid,userid,jsonstate FROM tb_user WHERE sfky=1 AND sysdate>ksrq AND uuid!=? AND pgid=?";
                        List<Map<String, Object>> list3 = jdbcTemplate.queryForList(str_sql, token, str_pgid);
                        ...
                        for (i = 0; i < len; i++)//对于每一个web用户
                        {
                            ...
                            str_sql = "select a.uuid,a.jsonstate,a.ispublic  from  tb_userunit a,tb_unittype b \n" +
                                    "where a.owneruuid=? and a.sfky=1 and pgid=?  and  a.unittype=b.uuid";
                            List<Map<String, Object>> list4 = jdbcTemplate.queryForList(str_sql, webUserUuid, str_pgid);
                ...
                case "giveuserstate_init": {//后面就不进行身份验证了
                    ...
                    WebSocketServer.PrivateTalk(JSON.toJSONString(obj_json), obj_json.getString("ChannelId"));
                    break;
                }
                //world通知其他用户新用户的进入
                case "bc_addanewuser": {
